...
def user_identity_data() -> dict:
    """
    Provide a valid Red Hat "User" identity payload for use in unit tests.
    
    The payload contains two top-level keys:
    - `identity`: includes `account_number`, `org_id`, `type` (set to `"User"`), and `user` with `user_id`, `username`, and `is_org_admin`.
    - `entitlements`: maps service names (for example, `"rhel"`, `"ansible"`, `"openshift"`) to objects with `is_entitled` and `is_trial`.
    
    Returns:
        dict: A dictionary suitable for JSON-encoding and base64-encoding into an `x-rh-identity` header.
    """
    return {
        "identity": {
            "account_number": "123",
...
class TestRHIdentityHealthProbeSkip:
    ...
    def _mock_configuration(
        ...
        """
        Patch the module-level configuration with a mock that sets authentication_configuration.skip_for_health_probes.
        
        Parameters:
            skip_for_health_probes (bool): Value to assign to the mocked configuration's authentication_configuration.skip_for_health_probes flag.
        """
...
class TestRHIdentityFieldValidation:
    ...
    def test_org_id_oversized_rejected(self, user_identity_data: dict) -> None:
        ...
        Expects RHIdentityData to raise an HTTPException with status_code 400 when identity.org_id length exceeds 256 characters.
        """
