...
def test_inference_default_provider_missing() -> None:
    """
    Checks that constructing InferenceConfiguration with only `default_model` set raises a ValueError.
    
    Asserts the error message equals "Default provider must be specified when default model is set".
    """
    with pytest.raises(
        ValueError,
