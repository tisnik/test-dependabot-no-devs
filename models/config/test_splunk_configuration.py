...
def token_file_fixture(tmp_path: Path) -> Path:
    """
    Create a temporary file named "token" containing the text "test-token" for use in tests.
...
def test_valid_enabled_configuration(token_file: Path) -> None:
    """
    Verify that an enabled SplunkConfiguration with all required fields is accepted and preserves the provided values.
    
    Asserts that each field (enabled, url, token_path, index, source, timeout, verify_ssl) on the created configuration equals the value passed to the constructor.
    """
