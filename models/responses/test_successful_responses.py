...
class TestModelsResponse:
    ...
    def test_openapi_response(self) -> None:
        """
        Verify ModelsResponse.openapi_response() produces a well-formed OpenAPI response with a JSON example.
        
        Asserts that the returned mapping has description "Successful response", uses ModelsResponse as the model, includes an 'application/json' content entry with an 'example' containing a 'models' list, and that the number of examples in the model JSON schema equals 1.
        """
...
class TestProvidersListResponse:
    ...
    def test_openapi_response(self) -> None:
        """
        Verify ProvidersListResponse.openapi_response() returns a Successful response structure and that the model schema contains exactly one example.
...
class TestReadinessResponse:
    ...
    def test_openapi_response(self) -> None:
        """
        Verify ReadinessResponse.openapi_response() returns the expected OpenAPI structure and examples.
        
        Asserts the returned mapping has description "Successful response", the `model` is ReadinessResponse, and `content["application/json"]` contains an "example". Also verifies the number of examples in ReadinessResponse.model_json_schema() equals 1.
        """
...
class TestFeedbackResponse:
    ...
    def test_openapi_response(self) -> None:
        """
        Verify FeedbackResponse.openapi_response() produces a valid OpenAPI response with an example.
        
        Asserts that the returned mapping has description "Successful response", uses FeedbackResponse as the model,
        contains an "application/json" content entry with an "example", and that the model's JSON schema exposes exactly one example.
        """
...
class TestStatusResponse:
    ...
    def test_constructor_feedback_disabled(self) -> None:
        """
        Verifies that StatusResponse stores a disabled feedback status and is an AbstractSuccessfulResponse.
        
        Asserts that `functionality` is "feedback", `status` equals {"enabled": False}, and the instance is a subclass of AbstractSuccessfulResponse.
        """
    ...
    def test_openapi_response(self) -> None:
        """
        Verify StatusResponse.openapi_response() returns a valid OpenAPI response with an example consistent with the model schema.
...
class TestAuthorizedResponse:
    ...
    def test_constructor_fields_required(self) -> None:
        ...
        Asserts that constructing AuthorizedResponse with no arguments, with only `username`, or with only `user_id` raises a pydantic.ValidationError.
        """
        with pytest.raises(ValidationError):
...
class TestConversationDeleteResponse:
    ...
    def test_openapi_response_missing_label(self) -> None:
        """
        Verifies that calling openapi_response() on a response model whose JSON schema examples lack a required "label" raises a SchemaError.
        
        Creates a ConversationDeleteResponse subclass with a malformed json_schema_extra.examples entry missing the "label" key and asserts openapi_response() fails with an error message containing "has no label".
        """
...
class TestConversationsListResponseV2:
    ...
    def test_constructor(self) -> None:
        """
        Validate that ConversationsListResponseV2 stores provided ConversationData entries correctly.
        
        Constructs a ConversationsListResponseV2 with a single ConversationData entry and asserts the response is an AbstractSuccessfulResponse, contains one conversation, and preserves the conversation_id.
        """
        conversations = [
            ConversationData(
                conversation_id="123e4567-e89b-12d3-a456-426614174000",
...
class TestFeedbackStatusUpdateResponse:
    ...
    def test_constructor(self) -> None:
        """
        Verify that FeedbackStatusUpdateResponse accepts and preserves a status dictionary and is a successful response.
        
        Asserts that constructing FeedbackStatusUpdateResponse with a status dict produces an AbstractSuccessfulResponse instance and that its `.status` equals the provided dictionary.
        """
        status_dict = {
...
class TestStreamingQueryResponse:
    ...
    def test_model_json_schema_has_examples(self) -> None:
        ...
        Asserts that the returned schema contains an "examples" key, that it has exactly one item, and that the item is a string.
        """
...
class TestMCPClientAuthOptionsResponse:
    ...
    def test_openapi_response(self) -> None:
        """
        Verify the OpenAPI response structure and example presence for MCPClientAuthOptionsResponse.
        
        Asserts that the model's JSON schema defines examples (exactly one), and that openapi_response() returns a mapping with description "Successful response", the response model set to MCPClientAuthOptionsResponse, and an "example" entry under content/application/json.
        """
