...
class TestUnauthorizedResponse:
    ...
    def test_constructor(self) -> None:
        """
        Verify UnauthorizedResponse constructs an AbstractErrorResponse with the expected HTTP 401 status and detail payload when a cause is provided.
    ...
    def test_openapi_response(self) -> None:
        """
        Validate UnauthorizedResponse.openapi_response() returns correct OpenAPI metadata and examples.
        ...
        "invalid signature", "invalid key", "missing claim", "invalid k8s token", and "invalid jwk token",
        and that the "missing header" example's `value.detail` contains the expected `response`
        and `cause` strings.
        """
...
class TestUnprocessableEntityResponse:
    ...
    def test_openapi_response(self) -> None:
        """
        Verify UnprocessableEntityResponse.openapi_response() returns an OpenAPI response with the expected description, model, and labeled examples.
        
        Asserts that the number of examples in the returned content matches the model's schema examples, that the three expected example labels ("invalid format", "missing attributes", "invalid value") are present, and that the "invalid format" example contains a `detail.response` equal to "Invalid request format".
        """
