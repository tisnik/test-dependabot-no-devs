...
def sample_request_fixture(sample_context: RlsapiV1Context) -> RlsapiV1InferRequest:
    """
    Create a reusable RlsapiV1InferRequest used in tests.
    
    Parameters:
        sample_context (RlsapiV1Context): Context to attach to the request.
    
    Returns:
        RlsapiV1InferRequest: Request initialized with question "How do I list files?", the provided context, and skip_rag set to True.
    """
...
class TestRlsapiV1Context:
    ...
    def test_constructor_defaults(self) -> None:
        ...
        Asserts that `stdin` is an empty string and that `attachments`, `terminal`, `systeminfo`, and `cla` are instantiated as their respective model types.
        """
...
def make_request_fixture() -> Any:
    """
    Create a test request builder for constructing RlsapiV1InferRequest instances with customizable context fields.
    
    The returned helper exposes a static build(question='q', stdin='', attachment='', terminal='') that returns an RlsapiV1InferRequest whose context contains the provided stdin, a single RlsapiV1Attachment with contents set to `attachment`, and a RlsapiV1Terminal with output set to `terminal`.
    ...
    class _RequestBuilder:  # pylint: disable=too-few-public-methods
        ...
        def build(
            ...
            """
            Builds an RlsapiV1InferRequest whose context contains the provided stdin, attachment contents, and terminal output.
...
def test_value_max_length(model: Callable, field: str, max_length: int) -> None:
    ...
    Constructs an instance with a string of exactly `max_length` characters and asserts the model accepts it, then attempts to construct the model with a value one character longer and asserts a `ValidationError` is raised with the message "should have at most {max_length} characters". Finally, confirms the created instance retains the valid value.
