...
def mock_session_fixture(mocker: MockerFixture) -> Any:
    """
    Create an aiohttp.ClientSession-like async mock whose post() call yields an async context-managed response with status 200.
    
    Parameters:
    	mocker (pytest_mock.MockerFixture): Fixture used to create AsyncMock objects.
    
    Returns:
    	AsyncMock: A mock session (`spec=aiohttp.ClientSession`) whose `post()` returns an async context manager that yields a response mock with `status = 200`.
    """
