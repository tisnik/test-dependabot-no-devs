...
async def test_authorized_dependency_unauthorized() -> None:
    """
    Validate that extract_user_token raises an HTTPException with status code 401 and the expected detail for missing or malformed Authorization headers.
    
    Checks two scenarios:
    - Missing Authorization header: exception.status_code == 401, detail["response"] == "Missing or invalid credentials provided by client", detail["cause"] == "No Authorization header found".
    - Invalid Authorization format: exception.status_code == 401, detail["response"] == "Missing or invalid credentials provided by client", detail["cause"] == "No token found in Authorization header".
    """
