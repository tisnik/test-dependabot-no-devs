...
def quota_scheduler(config: QuotaHandlersConfiguration) -> bool:
    ...
    Parameters:
        config (QuotaHandlersConfiguration): Configuration containing storage settings (sqlite or postgres), a list of limiter configurations, and scheduler.period in seconds. If configuration is invalid or no storage/limiters are configured, the scheduler will not start.
    
    Returns:
        bool: `True` if the scheduler started (unreachable in normal execution because the function enters an infinite loop), `False` if validation failed or a database connection could not be established.
    """
    if config is None:
...
def get_increase_quota_statement(config: QuotaHandlersConfiguration) -> str:
    """
    Select the SQL statement used to increase stored quota according to the configured database backend.
    
    Parameters:
        config (QuotaHandlersConfiguration): Configuration that indicates which storage backend (SQLite or PostgreSQL) is in use.
...
def get_reset_quota_statement(config: QuotaHandlersConfiguration) -> str:
    ...
    Returns:
        str: The SQLite reset SQL statement when `config.sqlite` is set, otherwise the PostgreSQL reset SQL statement.
    """
    if config.sqlite is not None:
...
def quota_revocation(
    ...
    """
    Apply configured quota updates for a single quota limiter using the provided database connection.
    
    Processes the given limiter: increases quota when `quota_increase` is set and resets initial quota when `initial_quota` is greater than zero, using the supplied SQL statements.
...
def increase_quota(
    ...
    Executes the provided SQL update statement on the given database connection to increment the quota value for the specified subject and period.
    
    Parameters:
        connection (Any): Database connection object (Postgres or SQLite) to execute the statement on.
        update_statement (str): SQL update statement that accepts parameters (increase_by, subject_id, period).
        subject_id (str): Identifier for the subject whose quota is modified (e.g., "u" for user, "c" for cluster).
...
def reset_quota(
    ...
    Parameters:
        ...
        update_statement (str): SQL statement that sets the quota value (expects parameters: new_value, subject_id, period).
...
def get_subject_id(limiter_type: str) -> str:
    ...
    Returns:
        str: `"u"` for a user limiter, `"c"` for a cluster limiter, or `"?"` if the type is unrecognized.
    """
    match limiter_type:
...
def connect(config: QuotaHandlersConfiguration) -> Any:
    ...
    Parameters:
        config (QuotaHandlersConfiguration): Configuration containing `postgres` or `sqlite` connection settings.
    
    Returns:
        A database connection suitable for quota operations, or `None` if neither Postgres nor SQLite is configured.
    """
...
def init_tables(connection: Any) -> None:
    """
    Create the quota table(s) required by the quota limiter on the provided database connection.
    
    Parameters:
        connection (Any): A DB-API compatible connection on which the quota table(s) will be created; changes are committed before returning.
    """
...
def start_quota_scheduler(configuration: Configuration) -> None:
    ...
    Parameters:
        configuration (Configuration): Global configuration whose `quota_handlers` attribute is passed to the scheduler thread.
    """
