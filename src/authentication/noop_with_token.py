...
class NoopWithTokenAuthDependency(
    ...
    def __init__(self, virtual_path: str = DEFAULT_VIRTUAL_PATH) -> None:
        """
        Initialize the no-op auth dependency with a virtual path and enable skipping user-id checks.
        
        Parameters:
            virtual_path (str): Virtual base path used for authorization context; defaults to DEFAULT_VIRTUAL_PATH.
    ...
    async def __call__(self, request: Request) -> tuple[str, str, bool, str]:
        """
        Return an identity tuple for development-mode no-op authentication.
        
        Returns:
            tuple[str, str, bool, str]: A 4-tuple containing:
                - user_id: The value of the "user_id" query parameter or DEFAULT_USER_UID if absent.
