...
class NoopAuthDependency(AuthInterface):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, virtual_path: str = DEFAULT_VIRTUAL_PATH) -> None:
        """
        Create a NoopAuthDependency configured with a virtual path and with user-ID checking disabled.
        
        Parameters:
            virtual_path (str): Virtual path context used by the dependency (defaults to DEFAULT_VIRTUAL_PATH).
        """
    ...
    async def __call__(self, request: Request) -> tuple[str, str, bool, str]:
        """
        Provide a no-op authentication result for a FastAPI request, bypassing real authentication and authorization checks.
        ...
        Returns:
            tuple[str, str, bool, str]: A 4-tuple containing:
                - user_id: the value of the "user_id" query parameter if present, otherwise DEFAULT_USER_UID.
