...
class RevokableQuotaLimiter(QuotaLimiter):
    ...
    def __init__(
        ...
        Parameters:
            configuration (QuotaHandlersConfiguration): Configuration object containing `sqlite` and `postgres` connection settings.
            initial_quota (int): The starting quota value assigned when a subject's quota is initialized or revoked.
            increase_by (int): Number of quota units to add when increasing a subject's quota.
            subject_type (str): Identifier for the kind of subject the limiter applies to (e.g., user, customer); when set to "c" the limiter treats subject IDs as empty strings.
    ...
    def available_quota(self, subject_id: str = "") -> int:
        ...
        Parameters:
            subject_id (str): Subject identifier. For limiters with subject_type "c", this value is ignored and treated as an empty string.
    ...
    def _read_available_quota(self, query_statement: str, subject_id: str) -> int:
        ...
        If no quota record exists for the given subject, initializes the quota and returns the limiter's initial quota.
        ...
        Returns:
            int: The available quota for the subject; `initial_quota` if a new record was initialized.
        """
    ...
    def revoke_quota(self, subject_id: str = "") -> None:
        ...
        Parameters:
        	subject_id (str): Identifier of the subject whose quota will be revoked. If the limiter's `subject_type` is `"c"`, this value is ignored and treated as an empty string.
        """
        if self.subject_type == "c":
            subject_id = ""
    ...
    def _revoke_quota(self, set_statement: str, subject_id: str) -> None:
        """
        Set the subject's available quota back to the configured initial quota and record the revocation timestamp.
        
        Parameters:
            set_statement (str): SQL statement that updates the available quota and `revoked_at` for a subject.
            subject_id (str): Identifier of the subject whose quota will be revoked.
    ...
    def increase_quota(self, subject_id: str = "") -> None:
        ...
        Parameters:
            subject_id (str): Identifier of the subject whose quota will be increased. When the limiter's `subject_type` is `"c"`, this value is normalized to the empty string and treated as a global/customer-level entry.
        """
        if self.subject_type == "c":
            subject_id = ""
    ...
    def _increase_quota(self, set_statement: str, subject_id: str) -> None:
        """
        Increase the stored quota for a subject by the configured increment and record the update timestamp.
        
        Executes the provided SQL statement with parameters (increase amount, update timestamp, subject_id, subject_type) and commits the transaction.
    ...
    def ensure_available_quota(self, subject_id: str = "") -> None:
        ...
        Parameters:
        	subject_id (str): Identifier of the subject to check. If this limiter's `subject_type` is `"c"`, the value is ignored and treated as an empty string.
        
        Raises:
        	QuotaExceedError: If the available quota for the subject is less than or equal to zero.
        """
        if self.subject_type == "c":
            subject_id = ""
    ...
    def consume_tokens(
        ...
        Deducts the sum of `input_tokens` and `output_tokens` from the subject's stored quota and persists the update to the configured database backend. For subject type "c", the `subject_id` is normalized to an empty string before performing the operation.
    ...
    def _consume_tokens(
        ...
        """
        Deduct the sum of input and output tokens from the subject's available quota and persist the update.
    ...
    def _init_quota(self, subject_id: str = "") -> None:
        ...
        Inserts a quota row for `subject_id` with both available and total quota set to the limiter's configured initial value and stamps the revocation timestamp. The operation writes to whichever backend(s) are configured (SQLite and/or PostgreSQL) and commits the transaction.
        
        Parameters:
            subject_id (str): Identifier of the subject whose quota to initialize. Defaults to empty string.
        """
        # timestamp to be used
        revoked_at = datetime.now()
