...
class QuotaLimiter(ABC):
    ...
    def revoke_quota(self) -> None:
        ...
        This operation removes or disables any remaining allowance so subsequent checks will report no available quota.
        """

    @abstractmethod
    def increase_quota(self) -> None:
        """
        Increase the available quota for the limiter's subject according to its configured increase policy.
        
        Updates persistent storage to add the configured quota increment to the subject's stored available quota.
        """
    ...
    def consume_tokens(
        ...
        Parameters:
            ...
            subject_id (str): Identifier of the subject (user or service) whose quota will be reduced. If omitted, applies to the default subject.
        """
    ...
    def __init__(self) -> None:
        ...
        Attributes:
            sqlite_connection_config (Optional[SQLiteDatabaseConfiguration]): SQLite connection configuration or `None` when not configured.
            postgres_connection_config (Optional[PostgreSQLDatabaseConfiguration]): PostgreSQL connection configuration or `None` when not configured.
        """
    ...
    def _initialize_tables(self) -> None:
        ...
        Implementations must ensure the database schema and indexes needed for storing and querying quota state exist; calling this method when the schema already exists should be safe (idempotent). Raise an exception on irrecoverable initialization failures.
        """

    # pylint: disable=W0201
    def connect(self) -> None:
        """
        Establish the configured database connection, initialize required tables, and enable autocommit.
        
        If a PostgreSQL or SQLite configuration is present, a connection to that backend will be created, then _initialize_tables() will be called to prepare storage. If table initialization fails, the connection is closed and the original exception is propagated.
        """
