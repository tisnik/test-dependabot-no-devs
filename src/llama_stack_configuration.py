...
class YamlDumper(yaml.Dumper):
    ...
    def increase_indent(self, flow: bool = False, indentless: bool = False) -> None:
        """
        Force block-style indentation for emitted YAML by ensuring the dumper never uses "indentless" indentation.
        
        Parameters:
            flow (bool): Whether the YAML flow style is being used; forwarded to the base implementation.
            indentless (bool): Ignored — this implementation always enforces indented block style.
        """
        _ = indentless
...
def generate_configuration(
    ...
    """
    Update a Llama Stack YAML configuration file by inserting BYOK RAG vector DB and provider entries when present.
    
    Reads the YAML configuration from `input_file`, and if `config.byok_rag` contains items, updates or creates the `vector_dbs` and `providers.vector_io` sections (preserving any existing entries) based on that BYOK RAG data, then writes the resulting configuration to `output_file`. If `config.byok_rag` is empty, the input configuration is written unchanged to `output_file`.
    ...
    Parameters:
        ...
        config (Configuration): Configuration object whose `byok_rag` list supplies BYOK RAG entries to be added.
    """
...
def construct_vector_dbs_section(
    ...
    Parameters:
        ls_config (dict[str, Any]): Existing Llama Stack configuration mapping used as the base; existing `vector_dbs` entries are preserved if present.
        byok_rag (list[ByokRag]): List of BYOK RAG definitions to be added to the `vector_dbs` section.
...
def construct_vector_io_providers_section(
    ...
    """
    Builds the providers/vector_io list for a Llama Stack configuration by preserving existing entries and appending providers derived from BYOK RAG entries.
    
    Parameters:
        ls_config (dict[str, Any]): Existing Llama Stack configuration dictionary; if it contains providers.vector_io, those entries are used as the starting list.
        byok_rag (list[ByokRag]): List of BYOK RAG specifications to convert into provider entries.
    
    Returns:
        list[dict[str, Any]]: The resulting providers/vector_io list containing the original entries (if any) plus one entry per item in `byok_rag`. Each appended entry has `provider_id` set to "byok_<vector_db_id>", `provider_type` set from the RAG item, and a `config` with a `kvstore` pointing to ".llama/<vector_db_id>.db", `namespace` as None, and `type` "sqlite".
    """
    output = []
