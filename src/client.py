...
class AsyncLlamaStackClientHolder(metaclass=Singleton):
    ...
    async def load(self, llama_stack_config: LlamaStackConfiguration) -> None:
        """
        Load and initialize the holder's AsyncLlamaStackClient according to the provided configuration.
        
        If `llama_stack_config.use_as_library_client` is True, a library-mode client is created using
        `llama_stack_config.library_client_config_path` and initialized before being stored. Otherwise,
        a service-mode client is created using `llama_stack_config.url` and optional `llama_stack_config.api_key`.
        The created client is stored on the instance for later retrieval via `get_client()`.
        
        Parameters:
            llama_stack_config (LlamaStackConfiguration): Configuration that selects client mode and provides
                either a library client config path or service connection details (URL and optional API key).
