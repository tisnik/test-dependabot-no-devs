...
class RolesResolver(ABC):  # pylint: disable=too-few-public-methods
    ...
    async def resolve_roles(self, auth: AuthTuple) -> UserRoles:
        ...
        Parameters:
            auth (AuthTuple): Authentication tuple (for example, a token and associated metadata) used to determine roles.
...
def unsafe_get_claims(token: str) -> dict[str, Any]:
    """
    Extract JWT payload claims without validating the token signature.
    
    Parses the token's second (payload) segment as URL-safe base64, decodes it, and returns the resulting JSON object as a dict.
...
class JwtRolesResolver(RolesResolver):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, role_rules: list[JwtRoleRule]):
        ...
        Parameters:
            role_rules (list[JwtRoleRule]): Ordered list of rules that map JWT claim matches to roles. Each rule specifies a JSONPath to evaluate, an operator to apply to matches, the roles to grant when the rule matches, and optional negation or regex matching.
        """
        self.role_rules = role_rules

    async def resolve_roles(self, auth: AuthTuple) -> UserRoles:
        """
        Determine user roles by evaluating configured JwtRoleRule objects against JWT claims extracted from the provided AuthTuple.
        
        Returns:
            roles (UserRoles): Set of role names derived from all configured rules that match the token's claims.
        """
    ...
    def evaluate_role_rules(rule: JwtRoleRule, jwt_claims: dict[str, Any]) -> UserRoles:
        ...
        Parameters:
            rule (JwtRoleRule): Rule containing a JSONPath expression, operator, and associated roles to grant when matched.
            jwt_claims (dict[str, Any]): Decoded JWT claims to evaluate against the rule's JSONPath.
        
        Returns:
            roles (set[str]): The set of roles from `rule.roles` if the rule matches `jwt_claims`, otherwise an empty set.
        """
        return (
            set(rule.roles)
    ...
    def _get_claims(auth: AuthTuple) -> dict[str, Any]:
        ...
        Returns:
            dict[str, Any]: Decoded JWT claims as a dictionary. Returns an empty dict when the token equals constants.NO_USER_TOKEN (guest). The token payload is decoded without validating the JWT signature.
        """
        _, _, _, token = auth
    ...
    def _evaluate_operator(
        ...
        - MATCH: any string item in match matches rule.compiled_regex (if compiled_regex is provided).
        
        Parameters:
            rule (JwtRoleRule): The role rule containing operator, value, negate flag, and optionally compiled_regex.
            match (Any): The value(s) produced by evaluating the JSONPath; may be a single value or an iterable of values for MATCH.
        
        Returns:
            bool: `true` if the operator evaluation (after applying negation when set) succeeds, `false` otherwise.
        """
        result = False
        match rule.operator:
...
class AccessResolver(ABC):  # pylint: disable=too-few-public-methods
    ...
    def check_access(self, action: Action, user_roles: UserRoles) -> bool:
        ...
        Returns:
            bool: `true` if at least one role in `user_roles` grants the requested `action`, `false` otherwise.
        """
    ...
    def get_actions(self, user_roles: UserRoles) -> set[Action]:
        ...
        Returns:
            set[Action]: The aggregated set of allowed actions for the given roles. If `ADMIN` is included in the aggregated actions, returns all available non-`ADMIN` actions.
        """
...
class GenericAccessResolver(AccessResolver):  # pylint: disable=too-few-public-methods
    ...
    def __init__(self, access_rules: list[AccessRule]):
        ...
        Parameters:
            access_rules (list[AccessRule]): List of access rules used to populate the resolver. Each rule's `role` is mapped to the union of its `actions`.
        
        Raises:
            ValueError: If any rule contains the `Action.ADMIN` action together with other actions.
        """
    ...
    def check_access(self, action: Action, user_roles: UserRoles) -> bool:
        ...
        If any role grants the ADMIN action, that role permits all non-ADMIN actions (ADMIN acts as a full override).
        ...
        Returns:
            true if at least one role permits the action or ADMIN override applies, false otherwise.
        """
    ...
    def get_actions(self, user_roles: UserRoles) -> set[Action]:
        ...
        Returns:
            allowed_actions (set[Action]): Set of actions the user may perform. If any role grants Action.ADMIN, returns every Action except Action.ADMIN.
        """
        actions = {
            action
            for role in user_roles
