...
class ConversationDeleteResponse(AbstractSuccessfulResponse):
    ...
    def __init__(self, *, deleted: bool, conversation_id: str) -> None:
        """
        Initialize a ConversationDeleteResponse and populate its public fields based on the deletion result.
        
        If `deleted` is True the response message is "Conversation deleted successfully"; otherwise it is "Conversation cannot be deleted".
    ...
    def openapi_response(cls) -> dict[str, Any]:
        """
        Build an OpenAPI-compatible FastAPI response dict using the model's examples from its JSON schema.
        
        Extracts labeled examples from the model's JSON schema `examples` and places them under `application/json` -> `examples`. The returned mapping includes a `description` ("Successful response"), the `model` (the class itself), and `content` containing the assembled examples.
        
        Returns:
            response (dict[str, Any]): A dict with keys `description`, `model`, and `content` suitable for FastAPI/OpenAPI response registration.
        
        Raises:
            SchemaError: If any example in the model's JSON schema is missing a required `label` or `value`.
        """
        schema = cls.model_json_schema()
        model_examples = schema.get("examples", [])
...
class AbstractErrorResponse(BaseModel):
    ...
    def get_description(cls) -> str:
        ...
        Returns:
            str: The class `description` attribute if present; otherwise the class docstring; if neither is present, an empty string.
        """
    ...
    def openapi_response(cls, examples: Optional[list[str]] = None) -> dict[str, Any]:
        """
        Builds an OpenAPI/FastAPI response dictionary that exposes the model's labeled examples.
        
        Extracts examples from the model's JSON schema and includes them as named application/json examples in the returned response mapping. If the optional `examples` list is provided, only examples whose labels appear in that list are included. Each included example is exposed under its label with a `value` containing a `detail` payload.
        
        Parameters:
            examples (Optional[list[str]]): If provided, restricts which labeled examples to include by label.
        
        Returns:
            dict[str, Any]: A response mapping with keys:
                - "description": the response description,
                - "model": the model class,
                - "content": a mapping for "application/json" to the examples object (or None if no examples).
...
class UnauthorizedResponse(AbstractErrorResponse):
    ...
    def __init__(self, *, cause: str):
        ...
        Initializes the error with a standardized response message and the provided cause, and sets the HTTP status to 401 Unauthorized.
        
        Parameters:
        	cause (str): Human-readable explanation of why the request is unauthorized (e.g. "missing token", "token expired").
        """
...
class ForbiddenResponse(AbstractErrorResponse):
    ...
    def conversation(
        ...
        Returns:
            ForbiddenResponse: Error response indicating the user is not permitted to perform the specified action on the conversation, with `response` and `cause` fields populated.
        """
        response = "User does not have permission to perform this action"
        cause = (
    ...
    def endpoint(cls, user_id: str) -> "ForbiddenResponse":
        ...
        Returns:
            ForbiddenResponse: Error response with a message and a cause referencing the given `user_id`.
        """
        response = "User does not have permission to access this endpoint"
    ...
    def feedback_disabled(cls) -> "ForbiddenResponse":
        ...
        Returns:
            ForbiddenResponse: Error response with `response` set to "Storing feedback is disabled" and `cause` set to "Storing feedback is disabled."
        """
        return cls(
            response="Storing feedback is disabled",
            cause="Storing feedback is disabled.",
    ...
    def model_override(cls) -> "ForbiddenResponse":
        """
        Create a ForbiddenResponse indicating that overriding the model or provider in the request is disallowed due to a missing permission.
        
        Returns:
            ForbiddenResponse: An error response with a user-facing message instructing removal of model/provider fields and a cause stating the missing `MODEL_OVERRIDE` permission.
        """
        return cls(
            response=(
                "This instance does not permit overriding model/provider in the "
    ...
    def __init__(self, *, response: str, cause: str):
        ...
        Parameters:
        	...
        	cause (str): Detailed cause or reason for the denial intended for logs or diagnostics.
        """
        super().__init__(
...
class QuotaExceededResponse(AbstractErrorResponse):
    ...
    def model(cls, model_name: str) -> "QuotaExceededResponse":
        ...
        Returns:
            QuotaExceededResponse: Response with a standard response message and a cause that includes the model name.
        """
        response = "The model quota has been exceeded"
    ...
    def from_exception(cls, exc: QuotaExceedError) -> "QuotaExceededResponse":
        """
        Constructs a QuotaExceededResponse representing the provided QuotaExceedError.
        
        @param exc: The QuotaExceedError instance whose message will be used as the cause.
        @returns: A QuotaExceededResponse initialized with a standard quota-exceeded message and the exception's text as the cause.
        """
        response = "The quota has been exceeded"
        cause = str(exc)
        return cls(response=response, cause=cause)
    ...
    def __init__(self, *, response: str, cause: str) -> None:
        ...
        Parameters:
            ...
            cause (str): Detailed cause or internal explanation for the quota exceedance; stored in the error detail.
...
class InternalServerErrorResponse(AbstractErrorResponse):
    ...
    def generic(cls) -> "InternalServerErrorResponse":
        ...
        @returns InternalServerErrorResponse: instance with a standard response message ("Internal server error") and a cause explaining an unexpected processing error.
        """
        return cls(
            response="Internal server error",
            cause="An unexpected error occurred while processing the request.",
    ...
    def configuration_not_loaded(cls) -> "InternalServerErrorResponse":
        """
        Create an InternalServerErrorResponse indicating the service configuration was not initialized.
        
        @returns InternalServerErrorResponse with response "Configuration is not loaded" and cause "Lightspeed Stack configuration has not been initialized."
        """
        return cls(
            response="Configuration is not loaded",
            cause="Lightspeed Stack configuration has not been initialized.",
    ...
    def feedback_path_invalid(cls, path: str) -> "InternalServerErrorResponse":
        """
        Create an InternalServerErrorResponse describing a failure to store feedback at the given filesystem path.
        
        Parameters:
            path (str): Filesystem directory where feedback storage was attempted.
        
        Returns:
            InternalServerErrorResponse: Error response with a response message "Failed to store feedback" and a cause indicating the failed directory.
        """
        return cls(
            response="Failed to store feedback",
    ...
    def query_failed(cls, backend_url: str) -> "InternalServerErrorResponse":
        ...
        Returns:
            InternalServerErrorResponse: An error response with response "Error while processing query" and cause "Failed to call backend: {backend_url}".
        """
        return cls(
            response="Error while processing query",
    ...
    def cache_unavailable(cls) -> "InternalServerErrorResponse":
        ...
        Returns:
            InternalServerErrorResponse: Error response with a message that the conversation cache is not configured and a corresponding cause.
        """
        return cls(
            response="Conversation cache not configured",
            cause="Conversation cache is not configured or unavailable.",
    ...
    def database_error(cls) -> "InternalServerErrorResponse":
        ...
        Returns:
            InternalServerErrorResponse: Instance with response "Database query failed" and cause "Failed to query the database".
        """
        return cls(
            response="Database query failed",
            cause="Failed to query the database",
    ...
    def __init__(self, *, response: str, cause: str) -> None:
        """
        Initialize the error response for internal server errors and set the HTTP status code to 500.
