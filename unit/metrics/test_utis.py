...
def test_update_llm_token_count_from_turn(mocker: MockerFixture) -> None:
    """
    Verifies that update_llm_token_count_from_turn increments LLM token metrics for received and sent tokens using the token counts produced by the formatter.
    
    Sets up a mock formatter that returns 3 tokens for the output and 2 tokens for the input, then asserts that:
