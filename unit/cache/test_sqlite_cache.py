...
class ConnectionMock:
    ...
    def __init__(self) -> None:
        ...
        The mock's cursor() method returns a CursorMock whose execute() raises sqlite3.Error, used to simulate a faulty connection in tests.
        """
    ...
    def cursor(self) -> Any:
        ...
        Returns:
            CursorMock: A mock cursor instance that simulates a DB cursor; its `execute` raises `sqlite3.Error` to emulate select-related errors.
        """
        return CursorMock()
...
def create_cache(path: Path) -> SQLiteCache:
    """
    Create a SQLiteCache configured to use a test.sqlite file inside the given directory.
...
def test_cache_initialization_wrong_connection() -> None:
    ...
    Asserts that attempting to create a SQLiteCache with a non-existent or inaccessible path raises an exception containing the text "unable to open database file".
    """
...
def test_get_operation_when_disconnected(tmpdir: Path) -> None:
    ...
    Sets the cache connection to None and asserts that calling `get` for a user and conversation raises a `CacheError` with the message "cache is disconnected".
    """
    cache = create_cache(tmpdir)
    cache.connection = None
...
def test_list_operation_when_connected(tmpdir: Path) -> None:
    ...
    Asserts that cache.list(USER_ID_1, False) produces a falsy result and that the returned value is a list.
    """
    cache = create_cache(tmpdir)
