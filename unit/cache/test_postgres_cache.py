...
class ConnectionMock:
    ...
    def cursor(self) -> None:
        """
        Simulate obtaining a database cursor and raise an OperationalError to represent a failed connection.
...
def postgres_cache_config() -> PostgreSQLDatabaseConfiguration:
    ...
    Returns:
        PostgreSQLDatabaseConfiguration: A configuration object with host, port, db, user, and a SecretStr password. Values are placeholders and not intended for real database connections.
    """
...
def test_cache_initialization(
    ...
    """
    Verifies that PostgresCache constructs successfully and exposes a non-None connection when psycopg2.connect is available.
...
def test_cache_initialization_connect_finalizer(
    ...
    Patches psycopg2.connect to avoid real DB access and makes PostgresCache.initialize_cache raise an exception; constructing PostgresCache must raise that exception.
    """
    # prevent real connection to PG instance
    mocker.patch("psycopg2.connect")
...
def test_get_operation_when_disconnected(
    ...
    """
    Verify that get() raises a CacheError with message "cache is disconnected" when the cache has no active database connection.
    """
    # prevent real connection to PG instance
    mocker.patch("psycopg2.connect")
    cache = PostgresCache(postgres_cache_config_fixture)
...
def test_delete_operation_operation_error(
    ...
    """
    Verifies that PostgresCache.delete raises a CacheError when the database connection fails during a delete operation.
    
    The test patches psycopg2.connect, injects a ConnectionMock that simulates a connection error, and asserts that calling delete(...) raises a CacheError containing the substring "delete".
    """
    # prevent real connection to PG instance
    mocker.patch("psycopg2.connect")
    cache = PostgresCache(postgres_cache_config_fixture)
