...
def noop_cache_config() -> ConversationHistoryConfiguration:
    """
    Provide a ConversationHistoryConfiguration configured for the NOOP cache type.
...
def memory_cache_config() -> ConversationHistoryConfiguration:
    """
    Provide a ConversationHistoryConfiguration configured for an in-memory conversation cache.
    
    Returns:
        ConversationHistoryConfiguration: Configuration with type set to in-memory and an InMemoryCacheConfig(max_entries=10).
    """
...
def postgres_cache_config() -> ConversationHistoryConfiguration:
    """
    Create a ConversationHistoryConfiguration configured for a PostgreSQL cache.
    
    Returns:
        ConversationHistoryConfiguration: Configuration with type set to POSTGRES and `postgres` populated with db="database", user="user", and password=SecretStr("password").
    """
    return ConversationHistoryConfiguration(
...
def sqlite_cache_config(tmpdir: Path) -> ConversationHistoryConfiguration:
    """
    Create a ConversationHistoryConfiguration for an SQLite cache using a temporary directory.
    
    Parameters:
        tmpdir (Path): Temporary directory path; the SQLite file will be created at `tmpdir / "test.sqlite"`.
    
    Returns:
        ConversationHistoryConfiguration: Configuration with `type` set to the SQLite cache constant and `sqlite` set to a SQLiteDatabaseConfiguration pointing to the test database path.
    """
...
def invalid_cache_type_config() -> ConversationHistoryConfiguration:
    """
    Create a ConversationHistoryConfiguration whose type is set to an invalid string to test factory validation.
...
def test_conversation_cache_in_memory(
    ...
    """
    Verify that CacheFactory returns an InMemoryCache when given an in-memory ConversationHistoryConfiguration.
    """
...
def test_conversation_cache_sqlite_improper_config(tmpdir: Path) -> None:
    """
    Verifies that a nil SQLite configuration causes CacheFactory.conversation_cache to raise a ValueError.
    
    Expects a ValueError with message containing "Expecting configuration for SQLite cache" when the ConversationHistoryConfiguration has type SQLITE but its sqlite field is None.
    """
    db_path = str(tmpdir / "test.sqlite")
    cc = ConversationHistoryConfiguration(
        type=CACHE_TYPE_SQLITE, sqlite=SQLiteDatabaseConfiguration(db_path=db_path)
...
def test_conversation_cache_postgres_improper_config() -> None:
    ...
    This test simulates an absent `postgres` config on a ConversationHistoryConfiguration with type `POSTGRES`
    and asserts that CacheFactory.conversation_cache raises ValueError with message containing
...
def test_conversation_cache_no_type() -> None:
    ...
    Asserts that calling CacheFactory.conversation_cache with a configuration whose `type` is None raises a ValueError with message "Cache type must be set".
    """
...
def test_conversation_cache_wrong_cache(
    ...
    This test calls CacheFactory.conversation_cache with a configuration whose type is invalid and asserts a ValueError is raised with a message containing "Invalid cache type".
    """
