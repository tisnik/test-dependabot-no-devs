...
def fixture_dummy_auth_tuple() -> AuthTuple:
    ...
    Returns:
        AuthTuple: A tuple of (user_id, username, is_admin_flag, token) where `user_id` and `username` are strings, `is_admin_flag` is a boolean, and `token` is a mock token string.
    """
...
class TestGetAuthorizationResolvers:
    ...
    def mock_configuration(self, mocker: MockerFixture) -> MockType:
        """
        Create a MagicMock configuration with empty authorization access rules and empty JWT role rules.
    ...
    def sample_access_rule(self) -> AccessRule:
        ...
        Returns:
            AccessRule: An AccessRule configured with role "test" and actions containing `Action.QUERY`.
        """
    ...
    def sample_role_rule(self) -> JwtRoleRule:
        """
        Create a JwtRoleRule that matches jsonpath "$.test" equal to "test" and assigns the "test" role.
        
        Returns:
            JwtRoleRule: Configured with jsonpath "$.test", operator `JsonPathOperator.EQUALS`, value "test", and roles ["test"].
        """
        return JwtRoleRule(
            jsonpath="$.test",
...
class TestPerformAuthorizationCheck:
    ...
    def mock_resolvers(self, mocker: MockerFixture) -> tuple[MockType, MockType]:
        ...
        Returns:
            tuple: (role_resolver, access_resolver)
                ...
                - access_resolver: a MagicMock with `check_access` returning `True` and `get_actions` returning `{Action.QUERY}`.
        """
    ...
    async def test_access_denied(
        ...
        """
        Verify that _perform_authorization_check raises an HTTP 403 with expected details when access is denied.
        
        Patches the authorization resolvers to deny access, invokes _perform_authorization_check for Action.ADMIN, and asserts the raised HTTPException has status 403 and a detail payload containing a `response` message about lack of permission and a `cause` that mentions the user is not authorized to access the endpoint.
        """
...
class TestAuthorizeDecorator:
    ...
    async def test_decorator_success(
        ...
        async def mock_endpoint(**_: Any) -> str:
            """
            Mock endpoint that returns a success indicator.
    ...
    async def test_decorator_failure(
        ...
        async def mock_endpoint(**_: Any) -> str:
            """
            Simulate an endpoint that returns a success indicator.
            
            Returns:
                str: "success" indicating the endpoint completed successfully.
            """
            return "success"
