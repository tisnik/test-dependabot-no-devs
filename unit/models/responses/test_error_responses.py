...
class TestBadRequestResponse:
    ...
    def test_openapi_response(self) -> None:
        """
        Verify that BadRequestResponse.openapi_response() produces an OpenAPI entry with the correct description, model reference, and JSON examples, and that the examples list matches the model schema's examples and contains a `conversation_id` example whose detail.response equals "Invalid conversation ID format".
        """
    ...
    def test_openapi_response_with_explicit_examples(self) -> None:
        """
        Verify BadRequestResponse.openapi_response returns only the specified example when explicit example labels are provided.
        
        Asserts that calling BadRequestResponse.openapi_response(examples=["conversation_id"]) produces application/json examples containing exactly one entry with the key "conversation_id".
        """
...
class TestForbiddenResponse:
    ...
    def test_factory_feedback_disabled(self) -> None:
        """
        Verifies that ForbiddenResponse.feedback_disabled() produces a 403 AbstractErrorResponse with the expected detail message and cause.
        """
