...
def test_authentication_configuration_rh_identity_but_insufficient_config() -> None:
    """
    Verify that selecting the RH Identity authentication module without providing a RHIdentityConfiguration raises a validation error.
...
def test_authentication_configuration_jwk_token_but_insufficient_config() -> None:
    """
    Verify that using the JWK token module with an insufficient `JwkConfiguration` triggers validation.
    
    Attempts to construct an `AuthenticationConfiguration` with `module=AUTH_MOD_JWK_TOKEN` and an empty `JwkConfiguration` must raise a `ValidationError` containing the text "JwkConfiguration".
    """
...
def test_authentication_configuration_jwk_broken_config() -> None:
    """
    Verify that accessing `jwk_configuration` raises a ValueError after the JWK config is removed.
    
    Creates an AuthenticationConfiguration with a JWK configuration, clears its `jwk_config`, and asserts that accessing `jwk_configuration` raises ValueError with message "JWK configuration should not be None".
    """
...
def test_authentication_configuration_supported() -> None:
    ...
    Asserts that the module is set to K8S, `skip_tls_verification` is False, and both `k8s_ca_cert_path` and `k8s_cluster_api` are None.
    """
    auth_config = AuthenticationConfiguration(
        module=AUTH_MOD_K8S,
        skip_tls_verification=False,
...
def test_authentication_configuration_in_config_rh_identity() -> None:
    """
    Verify that a Configuration with RH Identity authentication is constructed with the expected authentication fields.
