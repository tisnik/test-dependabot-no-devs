...
def test_jwt_role_rule_missing_attributes() -> None:
    ...
    Asserts that instantiating JwtRoleRule with no fields provided raises a ValidationError whose message contains "validation errors".
    """
...
def test_jwt_role_rule_correct_attributes() -> None:
    """
    Verify that a JwtRoleRule instantiated with valid attributes is created and does not compile a regex for non-MATCH operators.
    
    Asserts that an instance is returned and that `compiled_regex` is `None` when the operator is not `JsonPathOperator.MATCH`.
    """
    r = JwtRoleRule(
        jsonpath="$.id",
        negate=False,
...
def test_jwt_role_rule_invalid_json_path() -> None:
    """
    Verifies that constructing JwtRoleRule with an invalid JSONPath expression raises a ValidationError.
...
def test_jwt_role_rule_invalid_regexp() -> None:
    """
    Ensure creating a JwtRoleRule with an invalid regex pattern for the MATCH operator raises a ValidationError.
    
    Attempts to construct JwtRoleRule with operator set to JsonPathOperator.MATCH and a malformed regex value; the constructor must raise a ValidationError with message "Invalid regex pattern for MATCH operator".
    
    Raises:
        ValidationError: if the provided regex pattern for MATCH is invalid (message contains "Invalid regex pattern for MATCH operator").
    """
    with pytest.raises(
