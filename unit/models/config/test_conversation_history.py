...
def test_conversation_cache_no_type_specified() -> None:
    """
    Verify that a ConversationHistoryConfiguration created with no arguments has its type set to None.
    """
...
def test_conversation_cache_unknown_type() -> None:
    """
    Verify that providing an invalid conversation cache type raises a ValidationError with the expected message.
    
    The test constructs a ConversationHistoryConfiguration with an unsupported type value and asserts that validation fails with the message: "Input should be 'noop', 'memory', 'sqlite' or 'postgres'".
    """
    with pytest.raises(
        ValidationError,
...
def test_conversation_cache_correct_type_but_not_configured(subtests: SubTests) -> None:
    """
    Verify that specifying a cache type without providing its corresponding backend configuration raises a ValidationError with the expected message.
    
    Parameters:
        subtests (SubTests): pytest_subtests SubTests object used to create subtests for memory, sqlite, and postgres cases.
    """
...
def test_conversation_cache_no_type_but_configured(subtests: SubTests) -> None:
    """
    Verify that providing a backend configuration without specifying a conversation cache type raises a ValidationError.
    
    This test asserts that constructing ConversationHistoryConfiguration with a memory, sqlite, or postgres backend (but without setting the `type` field) fails with a ValidationError whose message is "Conversation cache type must be set when backend configuration is provided".
    """
...
def test_conversation_type_memory_wrong_config() -> None:
    """
    Verify that selecting the memory conversation cache raises validation errors for missing or invalid memory configuration.
    
    Asserts:
    - A missing `max_entries` field in the memory configuration raises a ValidationError with "Field required".
    - A `max_entries` value less than or equal to zero raises a ValidationError with "Input should be greater than 0".
    """
...
def test_conversation_type_sqlite_wrong_config() -> None:
    """
    Validate that selecting the SQLite conversation cache while supplying an incorrect backend configuration raises a validation error.
    
    This test asserts that when `type` is set to the SQLite cache but a `memory` configuration is provided instead of an `sqlite` configuration, model validation fails with a "Field required" error.
    """
    with pytest.raises(ValidationError, match="Field required"):
        _ = ConversationHistoryConfiguration(
            type=constants.CACHE_TYPE_SQLITE,
...
def test_conversation_type_postgres_wrong_config() -> None:
    ...
    This test provides an empty PostgreSQLDatabaseConfiguration and expects validation to fail with a "Field required" message.
    """
    with pytest.raises(ValidationError, match="Field required"):
        _ = ConversationHistoryConfiguration(
            type=constants.CACHE_TYPE_POSTGRES,
