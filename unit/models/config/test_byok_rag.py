...
def test_byok_rag_configuration_nondefault_values() -> None:
    """
    Verify ByokRag accepts and stores non-default configuration values.
    
    Asserts that rag_id, rag_type, embedding_model, embedding_dimension, and vector_db_id match the provided inputs and that db_path is converted to a Path.
    """
...
def test_byok_rag_configuration_empty_rag_type() -> None:
    ...
    Raises:
        ValidationError: if `rag_type` is an empty string; error message includes "String should have at least 1 character".
    """
