...
def test_cors_default_configuration() -> None:
    """
    Verify that a default CORSConfiguration instance has the expected default values.
...
def test_cors_custom_configuration_v3() -> None:
    """
    Verify that CORSConfiguration accepts a wildcard origin when credentials are disabled and preserves provided methods and headers.
    
    Creates a CORSConfiguration with allow_origins ["*"], allow_credentials False, and explicit allow_methods and allow_headers, then asserts the instance exists and its attributes match the provided values.
    """
    cfg = CORSConfiguration(
        allow_origins=["*"],
        allow_credentials=False,
...
def test_cors_improper_configuration() -> None:
    """
    Verify that constructing CORSConfiguration with a wildcard origin and credentials enabled raises a ValueError.
    
    Asserts the raised ValueError contains the message that `allow_credentials` cannot be true when `allow_origins` contains the '*' wildcard and advises using explicit origins or disabling credentials.
    """
    expected = (
