...
def test_quota_limiter_configuration_improper_value_1() -> None:
    """
    Verify that constructing a QuotaLimiterConfiguration with a negative `initial_quota` raises a ValueError with message "Input should be greater than or equal to 0".
    """
...
def test_quota_limiter_configuration_improper_value_2() -> None:
    ...
    Asserts that constructing a QuotaLimiterConfiguration with `quota_increase` less than zero raises a ValueError with the message "Input should be greater than or equal to 0".
    """
    with pytest.raises(ValueError, match="Input should be greater than or equal to 0"):
        _ = QuotaLimiterConfiguration(
            type="cluster_limiter",
...
def test_quota_limiter_configuration_improper_value_3() -> None:
    """
    Check that constructing QuotaLimiterConfiguration with an invalid `type` raises a ValueError with the expected message.
