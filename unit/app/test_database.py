...
def reset_database_state_fixture() -> Generator:
    """
    Reset the global database engine and session factory for the duration of a test.
    
    Sets `database.engine` and `database.session_local` to None before the test runs and restores their original values after the test completes.
    
    Returns:
        generator: A fixture generator that yields control to the test and restores global database state afterwards.
    """
...
def base_postgres_config_fixture() -> PostgreSQLDatabaseConfiguration:
    """
    Provide a baseline PostgreSQLDatabaseConfiguration for tests.
    
    Returns:
        PostgreSQLDatabaseConfiguration: Configuration with host "localhost", port 5432, database "testdb", user "testuser", password "testpass", and namespace "public".
    """
...
class TestInitializeDatabase:
    ...
    def _setup_common_mocks(
        ...
        *,
        ...
        Parameters:
            ...
            mock_sessionmaker (MockType): Mocked sessionmaker whose return value will be set to the mocked session-local factory.
        ...
        Returns:
            tuple[MockType, MockType]: A tuple (mock_engine, mock_session_local) where `mock_engine` is a mocked SQLAlchemy Engine and `mock_session_local` is the mocked session-local factory.
        """
        mock_engine = mocker.MagicMock(spec=Engine)
        mock_session_local = mocker.MagicMock()
    ...
    def _verify_common_assertions(
        ...
        *,
        ...
        """
        Assert that initialize_database set up the engine and session factory and that sessionmaker was invoked with the expected arguments.
        
        Parameters:
            mock_sessionmaker (MockType): Mock of the sessionmaker factory; expected to have been called with autocommit=False, autoflush=False, bind=mock_engine.
