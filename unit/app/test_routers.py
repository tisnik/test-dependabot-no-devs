...
class MockFastAPI(FastAPI):
    ...
    def __init__(self) -> None:  # pylint: disable=super-init-not-called
        """
        Create a mock FastAPI-like app and initialize its router registry.
        
        The instance attribute `routers` is initialized as an empty list that will store tuples of (router, prefix), where `prefix` is the route prefix string or `None`.
        """
    ...
    def include_router(  # pylint: disable=too-many-arguments
        ...
        *,
        ...
        """
        Register a router and its mount prefix on the mock FastAPI app for test inspection.
        ...
        Notes:
            Accepts additional FastAPI-compatible parameters for API compatibility but ignores them; only the (router, prefix) pair is recorded.
        """
        self.routers.append((router, prefix))

    def get_routers(self) -> list[Any]:
        """
        Retrieve the router objects registered on the mock FastAPI instance.
    ...
    def get_router_prefix(self, router: Any) -> Optional[str]:
        ...
        Returns:
            Optional[str]: The prefix string for the router, or `None` if the router was registered without a prefix.
...
def test_check_prefixes() -> None:
    ...
    Asserts that 16 routers are registered on a MockFastAPI instance and that each router's prefix matches the expected value (e.g., root, health, authorized, metrics use an empty prefix; most API routers use "/v1"; conversations_v2 uses "/v2").
    """
    app = MockFastAPI()
    include_routers(app)
