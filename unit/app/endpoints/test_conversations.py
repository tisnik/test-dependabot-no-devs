...
def dummy_request() -> Request:
    ...
    The returned Request has state.authorized_actions set to a set containing every member of Action.
    """
    request = Request(
        scope={
            "type": "http",
...
def create_mock_conversation(
    ...
    """
    Create a mock conversation object with the attributes used by the conversations list and detail tests.
    ...
    Parameters:
        ...
        created_at (str): ISO-formatted created-at timestamp to be returned by created_at.isoformat().
        last_message_at (str): ISO-formatted last-message timestamp to be returned by last_message_at.isoformat().
...
def mock_database_session(
    ...
    Parameters:
        ...
        query_result (Optional[list]): If provided, configures the session.query().all() and session.query().filter_by().all() to return this list.
    
    Returns:
        Mock: The mocked session object that will be yielded by the patched get_session context manager.
    """
...
def setup_configuration_fixture() -> AppConfig:
    ...
    Returns:
        AppConfig: An AppConfig instance initialized from a dictionary containing defaults suitable for tests (local service host/port, disabled auth and user-data collection, test Llama Stack API key and URL, and single worker).
    """
...
def mock_session_data_fixture() -> dict[str, Any]:
    """
    Provide a representative mock session data payload used by tests to simulate a conversation session.
    ...
    - turns: list of turn objects; each turn includes:
        ...
        - input_messages: list of input message objects with `content`, `role`, and optional `context`.
        - output_message: assistant response object with `content`, `role`, and auxiliary fields (e.g., `stop_reason`, `tool_calls`) that tests expect to be filtered by simplification logic.
    ...
    Returns:
        dict: A mock session data structure matching the shape produced by the Llama Stack client for use in unit tests.
    """
    return {
...
def mock_conversation_fixture() -> UserConversation:
    ...
    Returns:
        mock_conv (UserConversation): A UserConversation initialized with VALID_CONVERSATION_ID, user_id set to "another_user", message_count 2, last_used_model "mock-model", last_used_provider "mock-provider", and topic_summary "Mock topic".
    """
...
class TestGetConversationEndpoint:
    ...
    async def test_llama_stack_not_found_error(
        ...
        """
        Verify the GET /conversations/{conversation_id} handler raises an HTTP 404 when the Llama Stack client reports the session as not found.
        
        Asserts that the raised HTTPException contains a response message indicating the conversation was not found and a cause that includes "does not exist" and the conversation ID.
        """
        mock_authorization_resolvers(mocker)
        mocker.patch("app.endpoints.conversations.configuration", setup_configuration)
        mocker.patch("app.endpoints.conversations.check_suid", return_value=True)
...
class TestDeleteConversationEndpoint:
    ...
    async def test_sqlalchemy_error_in_delete(
        ...
        """
        Validate that if a SQLAlchemyError occurs while deleting a conversation, the endpoint raises an HTTPException with status 500 and a response detail containing "Database".
        """
        mock_authorization_resolvers(mocker)
        mocker.patch("app.endpoints.conversations.configuration", setup_configuration)
        mocker.patch("app.endpoints.conversations.check_suid", return_value=True)
