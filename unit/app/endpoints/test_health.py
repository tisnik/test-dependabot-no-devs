...
class TestGetProvidersHealthStatuses:
    ...
    async def test_get_providers_health_statuses(self, mocker: MockerFixture) -> None:
        """
        Verify get_providers_health_statuses returns a ProviderHealthStatus entry for each provider reported by the client.
        
        Mocks an AsyncLlamaStack client whose providers.list() returns three providers with distinct health dicts, then asserts the function produces three results with:
