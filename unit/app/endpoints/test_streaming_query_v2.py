...
def dummy_request() -> Request:
    ...
    Returns:
        Request: A FastAPI Request whose `state.authorized_actions` is set to a set of all `Action` members.
    """
...
async def test_streaming_query_endpoint_handler_v2_success_yields_events(
    ...
    async def fake_stream() -> AsyncIterator[SimpleNamespace]:
        ...
        Yields SimpleNamespace objects that emulate event frames from a streaming responses API, including:
        - a "response.created" event with a conversation id,
        - content and text delta events ("response.content_part.added", "response.output_text.delta"),
        - function call events ("response.output_item.added", "response.function_call_arguments.delta", "response.function_call_arguments.done"),
        - a final "response.output_text.done" event and a "response.completed" event.
        
        Returns:
            AsyncIterator[SimpleNamespace]: An async iterator that yields event-like SimpleNamespace objects representing the streamed response frames; the final yielded response contains an `output` attribute (an empty list) to allow shield violation detection in tests.
        """
...
async def test_streaming_query_endpoint_handler_v2_api_connection_error(
    ...
    def _raise(*_a: Any, **_k: Any) -> None:
        ...
        Raises:
            APIConnectionError: Raised every time the function is called; the exception's `request` is None.
        """
...
async def test_streaming_response_detects_shield_violation(
    ...
    async def fake_stream_with_violation() -> AsyncIterator[SimpleNamespace]:
        """
        Produce an async iterator of SimpleNamespace events that simulates a streaming response which ends with a safety refusal.
        ...
        Yields:
            AsyncIterator[SimpleNamespace]: Sequence of event objects in order:
                ...
                - type="response.completed" whose `response.output` contains a message object with a `refusal` field indicating a safety policy violation
        """
        yield SimpleNamespace(
...
async def test_streaming_response_no_shield_violation(
    ...
    async def fake_stream_without_violation() -> AsyncIterator[SimpleNamespace]:
        """
        Produce a deterministic sequence of streaming response events that end with a completed assistant message containing no refusal.
        ...
        - `response.completed` whose `response.output` contains an assistant message where `refusal` is `None`.
        
        Returns:
        	An iterator yielding SimpleNamespace objects representing the streaming events of a successful response with no refusal.
        """
        yield SimpleNamespace(
