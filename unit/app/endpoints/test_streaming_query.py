...
def mock_database_operations(mocker: MockerFixture) -> None:
    """
    Configure test mocks for conversation ownership validation and post-stream cleanup used by streaming-query tests.
    
    Parameters:
    	mocker (MockerFixture): Pytest-mock fixture used to patch functions. After calling this helper,
    	`validate_conversation_ownership` is patched to return `True` and
    	`cleanup_after_streaming` is patched to an async no-op.
    """
    mocker.patch(
...
def setup_configuration_fixture() -> AppConfig:
    ...
    The configuration uses a local service (localhost:8080), a test Llama Stack API key and URL, disables user transcript collection, and sets a noop conversation cache and empty MCP servers to avoid external dependencies.
...
async def test_streaming_query_endpoint_on_connection_error(
    ...
    def _raise_connection_error(*args: Any, **kwargs: Any) -> None:
        ...
        Accepts any positional and keyword arguments and always raises an APIConnectionError (with `request=None`), intended for use in tests to simulate a connection failure.
...
async def _test_streaming_query_endpoint_handler(mocker: MockerFixture) -> None:
    """
    Set up a simulated Llama Stack streaming response and verify the streaming-query endpoint emits the expected SSE events and referenced documents.
    
    Mocks an AsyncLlamaStack client and retrieve_response to produce a sequence of step_progress, step_complete, and turn_complete chunks, invokes streaming_query_endpoint_handler, and asserts that the returned StreamingResponse contains SSE start/token/end events, the final LLM answer, seven streamed chunks, and two referenced documents with the second titled "Doc2".
    """
...
async def test_retrieve_response_vector_db_available(
    ...
    """
    Verifies that retrieve_response detects available vector databases and invokes the agent with appropriate toolgroups for a streaming query.
    
    Mocks an agent and client with one vector database present, patches configuration and agent retrieval, then calls retrieve_response and asserts:
    - a streaming response object is returned (non-None),
    - the conversation ID returned matches the agent's ID,
    - the agent's create_turn is called once with the user message, streaming enabled, no documents, and toolgroups derived from the detected vector database.
    """
...
async def test_retrieve_response_two_available_shields(
    ...
    """
    Verifies retrieve_response uses available shields and starts a streaming turn with expected arguments.
    
    Patches configuration and agent retrieval to provide a mocked client and agent with two shields available, then calls retrieve_response and asserts:
    - a non-None response is returned and the conversation ID matches the mocked agent value,
    - the agent's create_turn is invoked once with the user's message, the mocked session_id, an empty documents list, stream=True, and toolgroups=None.
    """
...
async def test_retrieve_response_with_two_attachments(
    ...
    """
    Verifies that retrieve_response converts request attachments into document inputs, calls the agent with streaming enabled, and returns the agent response and conversation id.
...
def test_stream_build_event_step_progress_tool_call_tool_call() -> None:
    """
    Verify that stream_build_event formats a step_progress event containing a ToolCall delta into an OLS-compatible server-sent event.
    
    Asserts that the produced event includes the SSE data prefix, uses the "tool_call" event type, contains the tool name as the `token` value, and includes the expected `id` value while omitting the role field for OLS compatibility.
    """
    # Create a properly nested chunk structure
    # We cannot use 'mock' as 'hasattr(mock, "xxx")' adds the missing
    # attribute and therefore makes checks to see whether it is missing fail.
...
async def test_streaming_query_endpoint_rejects_model_provider_override_without_permission(
    ...
    """
    Verify the streaming query endpoint rejects requests that attempt to override the model or provider when the caller lacks the MODEL_OVERRIDE permission.
    
    This test patches configuration and authorization resolvers so MODEL_OVERRIDE is not granted, sends a QueryRequest including `model` and `provider`, and asserts the handler raises an HTTPException with status 403 whose detail contains a response message advising removal of the model and provider fields.
    """
    cfg = AppConfig()
    cfg.init_from_dict(
        {
