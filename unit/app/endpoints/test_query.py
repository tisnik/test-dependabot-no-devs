...
def dummy_request() -> Request:
    ...
    The returned Request has a minimal HTTP scope and a `state.authorized_actions` attribute initialized
    to a set containing all members of the `Action` enum, suitable for use in unit tests that require
...
def mock_metrics(mocker: MockerFixture) -> None:
    """
    Configure the provided pytest-mock `mocker` to stub token metrics and related metrics counters used by query endpoint tests.
    
    Patches the token metrics extraction helper and the LLM metrics counters so tests can run without emitting real metrics.
    """
    mocker.patch(
...
def mock_database_operations(mocker: MockerFixture) -> None:
    ...
    This applies test-time patches so that conversation ownership checks succeed, persistence of conversation
    details is stubbed out, and `get_session` returns a context-manager mock whose `query(...).filter_by(...).first()`
    returns `None`.
...
async def _test_query_endpoint_handler(
    ...
    """
    Exercise the query_endpoint_handler and assert observable outcomes for a typical successful request.
    
    Calls query_endpoint_handler with mocked dependencies and verifies the returned response and conversation_id match the agent summary, that a CacheEntry with referenced documents is stored, and that transcript storage is invoked only when transcripts are enabled in configuration.
    
    Parameters:
        store_transcript_to_file (bool): When True, configuration reports transcripts enabled and the test asserts store_transcript is called with expected arguments; when False, asserts store_transcript is not called.
    """
...
def test_select_model_and_provider_id_from_configuration(mocker: MockerFixture) -> None:
    """
    Verify that select_model_and_provider_id selects the default model and provider from configuration when the request does not specify them.
    
    Patches configuration defaults to "default_provider" and "default_model", provides a model list containing the configured default, and asserts the returned llama_stack_model_id, model_id, and provider_id match those configured values.
    """
    mocker.patch(
        "metrics.utils.configuration.inference.default_provider",
        "default_provider",
...
async def test_retrieve_response_one_available_shield(
    ...
    class MockShield:
        ...
        def __init__(self, identifier: str) -> None:
            ...
            Parameters:
                identifier (str): The identifier for this instance; saved to the `identifier` attribute.
            """
...
async def test_retrieve_response_with_one_attachment(
    ...
    """
    Verifies that retrieve_response includes a single attachment as a document when calling the agent and returns the LLM response and conversation id.
    ...
    - The agent.create_turn is invoked once with the attachment converted to a document dict containing `content` and `mime_type`, and with the expected session_id, messages, stream, and toolgroups.
    """
    mock_client, mock_agent = prepare_agent_mocks
    mock_agent.create_turn.return_value.output_message.content = "LLM answer"
    mock_client.shields.list.return_value = []
...
async def test_query_endpoint_handler_on_connection_error(
    ...
    """
    Verifies that query_endpoint_handler raises an HTTPException with status 503 when connecting to Llama Stack fails and that the failure metric is incremented.
    
    The test simulates an APIConnectionError from the Llama Stack client, calls query_endpoint_handler with a simple QueryRequest, and asserts that:
...
async def test_query_endpoint_rejects_model_provider_override_without_permission(
    ...
    """
    Verifies the /query endpoint rejects requests that include model/provider overrides when the caller lacks MODEL_OVERRIDE permission.
    
    Asserts that calling the endpoint with a QueryRequest containing `model` and `provider` raises an HTTP 403 with a JSON detail containing the specific permission-denied message.
    """
