...
def user_identity_data() -> dict:
    ...
    Returns:
        identity_data (dict): A dictionary with two top-level keys:
            - "identity": contains "account_number", "org_id", "type" (set to "User"), and "user" (with "user_id", "username", "is_org_admin").
            - "entitlements": maps service names (e.g., "rhel", "ansible", "openshift") to entitlement objects with "is_entitled" and "is_trial".
    """
    return {
        "identity": {
            "account_number": "123",
...
def create_auth_header(identity_data: dict) -> str:
    ...
    Parameters:
        identity_data (dict): Identity payload (serializable to JSON) containing identity, user/system fields, and optional entitlements.
...
def create_request_with_header(header_value: Optional[str]) -> Request:
    ...
    Parameters:
        header_value (Optional[str]): Base64-encoded identity header value to set. If `None` or empty, the returned request will have no headers.
    
    Returns:
        Request: A mocked Request object whose `headers` contains `{"x-rh-identity": header_value}` when a value is provided, or an empty dict otherwise.
    """
...
class TestRHIdentityData:
    ...
    def test_validate_entitlements(
        ...
        Creates an RHIdentityData instance from the provided identity dictionary and required_entitlements; asserts that calling validate_entitlements raises an HTTPException with status code 403 containing expected_error when should_raise is True, and does not raise when should_raise is False.
        """
