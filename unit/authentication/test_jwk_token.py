...
def token_header(single_key_set: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    Returns:
        dict: JWT header with keys `"alg": "RS256"`, `"typ": "JWT"`, and `"kid"` set to the first key's `kid`.
    """
...
def token_payload() -> dict[str, Any]:
    ...
    Returns:
        dict: A mapping with keys "user_id", "username", "exp", and "iat"; "exp" and "iat" are UNIX timestamps (seconds since epoch).
    """
    return {
...
def another_single_key_set() -> list[dict[str, Any]]:
    ...
    Returns:
        list[dict[str, Any]]: A list containing one key dict with keys `private_key`, `public_key`, and `kid`.
    """
    return [make_key()]
...
def valid_token(
    ...
    Parameters:
        single_key_set (list[dict[str, Any]]): List of key dicts where the first entry must contain a 'private_key' used to sign the token.
...
def make_signing_server(
    ...
    """
    Create and patch a mocked aiohttp.ClientSession that serves a JWKS response derived from the provided key set.
    
    Parameters:
        mocker (pytest.MockerFixture): Pytest mocker used to patch aiohttp.ClientSession.
        key_set (list[dict[str, Any]]): List of signing key dicts; each item must include a `private_key` with an `as_dict(private=False)` method and a `kid` value.
        algorithms (list[str]): List of JWK `alg` values to assign to each corresponding key in `key_set`.
    
    Returns:
        Any: The mocked ClientSession class (the value assigned to the patched `aiohttp.ClientSession`). The mock is configured so that:
          ...
          - The response's `json()` returns `{"keys": keys}` where each key is the public JWK derived from `private_key.as_dict(private=False)` extended with `kid` and `alg`.
...
def mocked_signing_keys_server(
    ...
    Parameters:
        ...
        single_key_set (list[dict[str, Any]]): A list containing one JWK dict (public key representation) that will be returned by the mocked JWKS endpoint.
    """
...
def default_jwk_configuration() -> JwkConfiguration:
    ...
    The returned configuration uses a mocked JWKS URL and a JwtConfiguration that maps the user identifier to the `user_id` claim and the username to the `username` claim.
    
    Returns:
        JwkConfiguration: Configuration with a mocked JWKS URL and default claim mappings (`user_id` and `username`).
    """
...
def dummy_request(token: str) -> Request:
    ...
    Returns:
        request (Request): FastAPI Request object with the Authorization header set to "Bearer <token>".
    """
    return Request(
        scope={
            "type": "http",
...
def no_token_request() -> Request:
    ...
    Returns:
        Request: A request object with an HTTP scope and an empty headers list (no Authorization present).
    """
    return Request(
        scope={
            "type": "http",
...
def set_auth_header(request: Request, token: str) -> None:
    ...
    This mutates request.scope["headers"] to remove any existing Authorization header and append a new one using the provided token value. The token parameter should be the full header value (for example, "Bearer <token>").
...
def expired_token(
    ...
    Parameters:
        single_key_set (list[dict]): A list of key dicts; the first element's `private_key` is used to sign the token.
        token_header (dict): JWT header values to include in the token.
        token_payload (dict): JWT payload values; this function overwrites `exp` to a past timestamp.
...
async def test_expired(
    ...
    Asserts that calling the dependency with an expired token raises an HTTPException with status code 401 and a message containing "Token has expired".
    """
    _ = mocked_signing_keys_server
...
def invalid_token(
    ...
    """
    Create a JWT signed with a key different from the expected signing keys for use in invalid-signature tests.
    
    Parameters:
        another_single_key_set (list[dict[str, Any]]): A key set whose first entry's "private_key" will be used to sign the token; should not match the verifier's keys.
...
def no_user_id_token(
    ...
    The token is encoded using the provided header and the first private key in `single_key_set`; the supplied `token_payload` is modified in-place to remove `user_id`.
...
def custom_claims_token(
    ...
    Parameters:
        single_key_set (list[dict[str, Any]]): List of signing key dicts; the first entry's `private_key` is used to sign the token.
        token_payload (dict[str, Any]): Base payload; `user_id` and `username` are replaced with `id_of_the_user` and `name_of_the_user`.
...
def token_header_256_2(multi_key_set: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    Parameters:
        multi_key_set (list[dict[str, Any]]): List of JWK-like dicts where each dict contains a `"kid"` entry; the second entry (index 1) is used.
    
    Returns:
        dict[str, Any]: JWT header with keys `"alg": "RS256"`, `"typ": "JWT"`, and `"kid"` taken from `multi_key_set[1]["kid"]`.
    """
...
def token_header_384(multi_key_set: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    Parameters:
        multi_key_set (list[dict[str, Any]]): A list of JWK-like dicts; must contain at least three entries. The `kid` from the item at index 2 is used.
...
def multi_key_set() -> list[dict[str, Any]]:
    ...
    Each dictionary contains the generated key pair and identifier fields used by tests (e.g., `private_key`, `public_key`, and `kid`).
...
def valid_tokens(
    ...
    """
    Generate three valid JWTs signed by the three keys in the provided multi-key set using the given headers and payload.
    
    Returns:
        tuple[str, str, str]: A tuple of JWT strings (token1, token2, token3) signed with multi_key_set[0] (RS256, header token_header_256_1), multi_key_set[1] (RS256, header token_header_256_2), and multi_key_set[2] (RS384, header token_header_384), respectively.
    """
...
def valid_tokens_no_kid(
    ...
    """
    Generate three valid JWTs signed by the three keys in multi_key_set, with headers that omit the `kid`.
    
    Returns:
        tuple[str, str, str]: Tuple of JWT strings in order (RS256 signed with first key, RS256 signed with second key, RS384 signed with third key).
    """
    key_for_256_1 = multi_key_set[0]
    key_for_256_2 = multi_key_set[1]
    key_for_384 = multi_key_set[2]
...
def multi_key_signing_server(
    ...
    Creates and returns a mock aiohttp signing-keys server wired to the provided `multi_key_set` and configured to advertise algorithms ["RS256", "RS256", "RS384"].
    ...
    Returns:
        A mock object that simulates an aiohttp client/session which, when queried, yields a response containing the configured JWKs.
    """
