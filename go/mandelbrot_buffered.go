...
// calcMandelbrot renders the Mandelbrot set and writes an ASCII PPM (P3) image to standard output.
// 
// The image is mapped to the complex plane region [-2, 1] × [-1.5, 1.5]. For each pixel the function
// iterates z = z^2 + c up to maxiter and uses the iteration count `i` (escape iteration or maxiter)
// to select a color from `palette` (writes palette[i] as the pixel RGB triple).
//
// width and height are the image dimensions in pixels. palette should provide at least maxiter+1
// entries so that an index equal to maxiter is valid. The output is written to stdout in P3 PPM format.
...
// and invokes calcMandelbrot to render the Mandelbrot set to stdout in PPM P3 format.
