...
// calcMandelbrot renders the Mandelbrot set into img using the provided color palette.
//
// It maps the complex plane rectangle with real axis from -2.0 to +1.0 and imaginary
// axis from -1.5 to +1.5 onto the image of size width×height. For each pixel the
// function performs up to maxiter iterations of z = z^2 + c and uses the iteration
// count `i` to index into palette (palette[i]) to pick the pixel color.
//
// The function writes RGBA pixels directly into img and prints the current imaginary
// coordinate (cy) for each completed row. The caller must ensure palette has at least
// maxiter+1 entries; otherwise indexing palette[i] may panic. There is no internal
// error handling.
