...
// calcMandelbrot generates a Mandelbrot fractal and writes it as an ASCII PPM (P3) image to standard output.
// The image size is width×height and each pixel's color is chosen by the number of iterations up to maxiter.
// The complex plane covered is approximately (cx,cy) ∈ [-2.0, 1.0] × [-1.5, 1.5].
// The provided palette must contain at least maxiter+1 entries because the iteration count can equal maxiter for points that do not escape.
