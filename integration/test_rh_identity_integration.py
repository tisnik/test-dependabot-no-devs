...
def client() -> Generator[TestClient, None, None]:
    """
    Provide a TestClient for the FastAPI application configured with the RH Identity test configuration.
    
    Sets the LIGHTSPEED_STACK_CONFIG_PATH environment variable to "tests/configuration/rh-identity-config.yaml", loads that configuration, imports the application, yields a TestClient connected to the app, and restores the original LIGHTSPEED_STACK_CONFIG_PATH after the fixture completes.
...
def user_identity_json() -> dict:
    """
    Provides a valid Red Hat Identity payload representing a user.
    
    Returns:
        dict: A JSON-compatible mapping with keys:
            - "identity": mapping containing "account_number", "org_id", "type" == "User" and a "user" mapping with "user_id", "username", and "is_org_admin".
            - "entitlements": mapping of product entitlements (e.g., "rhel", "ansible", "openshift"), each with "is_entitled" and "is_trial" boolean flags.
    """
    return {
        "identity": {
            "account_number": "123",
...
def system_identity_json() -> dict:
    ...
    Returns:
        dict: JSON with keys:
            - "identity": contains "account_number", "org_id", "type" set to "System", and "system" with "cn".
...
def encode_identity(identity_json: dict) -> str:
    """
    Encode an identity JSON object into a base64-encoded UTF-8 string.
...
class TestRHIdentityIntegration:
    ...
    def test_valid_user_identity(
        ...
        """
        Verify that a GET to /api/v1/conversations with a valid User RH Identity is accepted.
        
        Sends the provided User identity encoded in the `x-rh-identity` header and asserts the response status code is `200` (success) or `404` (no conversations).
        """
