...
def _load_openapi_spec_from_file() -> dict[str, Any]:
    ...
    If the configured file is present, its contents are parsed as JSON and returned as a dictionary.
...
def open_api_spec_from_url() -> dict[str, Any]:
    """
    Provides the OpenAPI specification loaded from the running application's /openapi.json endpoint.
...
def _check_openapi_top_level_info(spec: dict[str, Any]) -> None:
    """
    Validate top-level fields and metadata of an OpenAPI specification.
    ...
    Raises:
        AssertionError: If any required top-level field is missing or does not match the expected value.
    """
...
def _check_server_section_present(spec: dict[str, Any]) -> None:
    """
    Verify the OpenAPI document contains a non-empty 'servers' list.
...
def _check_paths_and_responses_exist(
    ...
    """
    Verify that a path, an HTTP method, and the specified response status codes exist in the OpenAPI specification.
    
    Parameters:
        spec (dict): The OpenAPI document parsed as a dictionary.
        path (str): The API path (e.g., "/items/{id}") to check under `spec["paths"]`.
        method (str): The HTTP method (lowercase, e.g., "get", "post") expected for the path.
        expected_codes (set[str]): Set of HTTP response status codes (as strings) expected to appear in the operation's `responses`.
    
    Raises:
        AssertionError: If the path is missing, the method operation is missing or not an object, or any expected response code is absent.
    """
...
def test_openapi_top_level_info_from_file(spec_from_file: dict[str, Any]) -> None:
    """
    Validate the top-level metadata of the OpenAPI specification loaded from file.
    
    Asserts that the OpenAPI version, info (title and version), contact, and license fields meet the repository's expected values.
    
    Parameters:
        spec_from_file (dict[str, Any]): OpenAPI specification dictionary loaded from docs/openapi.json.
    """
...
def test_openapi_top_level_info_from_url(spec_from_url: dict[str, Any]) -> None:
    """
    Validate the top-level fields of the OpenAPI specification obtained from the running application.
    
    Asserts that the OpenAPI version, info section (including title, version, and contact), and license (name and URL) meet the project's expectations used by the tests.
    
    Parameters:
        spec_from_url (dict[str, Any]): OpenAPI document parsed from the application's /openapi.json endpoint.
    """
...
def test_servers_section_present_from_url(spec_from_url: dict[str, Any]) -> None:
    """Verify the OpenAPI document served by the app includes a non-empty `servers` section."""
...
def test_paths_and_responses_exist_from_file(
    ...
    """
    Verify that the given path and HTTP method are defined in the provided OpenAPI specification and that the operation contains all expected response status codes.
    ...
    Parameters:
        ...
        expected_codes (set[str]): Set of expected HTTP response status codes as strings (e.g., {"200", "404"}).
    """
...
def test_paths_and_responses_exist_from_url(
    ...
    """
    Verify that the OpenAPI spec served at /openapi.json contains the given path and HTTP method and that the operation declares the specified response status codes.
    
    Parameters:
        path (str): OpenAPI path string to check (for example, "/items/{id}").
        method (str): HTTP method name for the operation to check (e.g., "get", "post"); case-insensitive.
        expected_codes (set[str]): Set of response status code strings expected to be present for the operation (for example, {"200", "404"}).
    """
