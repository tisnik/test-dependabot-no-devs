...
def main() -> int:
    ...
    Parses command-line arguments (--query, --system-prompt, --url, --timeout), POSTs a JSON payload with the query and system prompt to the configured endpoint, and prints the returned "response" value followed by the request elapsed time. Error diagnostics are printed to stderr.
    
    Returns:
        int: Exit code where `0` indicates success, `1` indicates an HTTP/request failure, `2` indicates the server response was not valid JSON, and `3` indicates the JSON response did not contain a `"response"` field.
    """
